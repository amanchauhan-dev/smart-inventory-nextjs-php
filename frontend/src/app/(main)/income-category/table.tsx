...
function ExpenseCategoryTable() {
    ...
    const { refreshIncomeCategories, refreshIncomeCategoriesFlag } = useRefresh()
    const [dataToUpdate, setDataToUpdate] = useState<IncomeCategory | null>(null)
    ...
    useEffect(() => {
        ; (async () => {
            setLoading(true);
            try {
                const { data, status } = await api.get(`/income-categories?offset=${offset}&limit=${limit}`)
                if (status == 200) {
    ...
    }, [refreshIncomeCategoriesFlag, setTotalRecords, limit, offset])


    const onUpdate = (data: IncomeCategory) => {
    ...
    const handleDelete = async () => {
        ...
        setOpenAlert(false)
        try {
            const { data, status } = await api.delete(`/income-categories/${idToDelete}`)
            if (status == 200) {
    ...
    return (
        <div className="border rounded-lg overflow-hidden">
            ...
            <UpdateDialogForm refresh={refreshIncomeCategories} open={openUpdate} setOpen={setOpenUpdate} data={dataToUpdate} />
